...
public class Menu : MonoBehaviour {
    ...
    private MenuEditCharacter menuEditCharacter;
    ...
    public List<Sprite> HairStylesLocal;
    public Sprite CharHead;
    public Sprite CharArms;
    public static int HairStyle = 0;
    public static Vector3 HairColor = new Vector3(0.7f, 0.4f, 0.3f);
    public static Vector3 TopColor = new Vector3(0.4f, 0.4f, 0.8f);
	...
	private string creditsText = "<b>Lead Designer/Programmer</b>\nKeirron Stach\n\n" +
		...
		"<b>QA Testing</b>\nRyan McCann, Scott McCann\nSam Dalby, Joey Testo\n\n" +
		"<b>Special Thanks</b>\nKarleigh Brown\nSean Stach\nJessica Mueller";
	...
	void Start () {
		...
		{
            string profilePath = MenuProfile.GetProfilePath(PlayerPrefs.GetString("Profile"));
            if (File.Exists(profilePath))
            {
                MainProfile = Profile.LoadProfile(profilePath);
            }
            else
            {
                MainProfile = menuProfile.GetProfile(0);
            }
		}
        ...
        if(PlayerPrefs.HasKey("HairStyle"))
        {
            ...
            float r = PlayerPrefs.GetFloat("HairR");
            float g = PlayerPrefs.GetFloat("HairG");
            float b = PlayerPrefs.GetFloat("HairB");
            HairColor = new Vector3(r, g, b);
            r = PlayerPrefs.GetFloat("TopR");
            g = PlayerPrefs.GetFloat("TopG");
            b = PlayerPrefs.GetFloat("TopB");
            TopColor = new Vector3(r, g, b);
        }
	}
	...
	void OnGUI()
	{
        ...
        GUI.skin.textField.fontSize = (int)(0.07f * h);
        GUI.skin.button.fontSize = (int)(0.02f * h);
        GUI.skin.label.fontSize = (int)(0.02f * h);
        ...
        if(CurrentScreen == ScreenType.EditCharacter)
        {
            if (menuEditCharacter != null)
                menuEditCharacter.Draw();
            else
                SwitchScreen(ScreenType.MainMenu);
        }
		...
		{
			buttonHeight = (Screen.height/2)/8;
			buttonWidth = buttonHeight * 3.5f;;
			float buttonDivider = buttonHeight / 7;


			
			buttonI = 0;
            float estY = Screen.height/2;
			...
			float welcomeH = Screen.height*0.05f;
			float welcomeX = Screen.width/2 - welcomeW/2;
			float welcomeY = logoY + logoH*1.1f;

            GUIStyle welcome = Blurift.BluStyle.CustomStyle(LabelCenter, welcomeH *0.4f);
            GUIStyle button = Blurift.BluStyle.CustomStyle(GUI.skin.button, buttonHeight * 0.35f);

			GUI.DrawTexture(new Rect(logoX,logoY,logoW,logoH), logo);
			GUI.Label(new Rect(welcomeX,welcomeY,welcomeW,welcomeH), "Version: " + Game.VERSION + "\nHello " + MainProfile.Name, welcome);

			if(GUI.Button(new Rect(x,y + (buttonI*(buttonHeight+buttonDivider)),buttonWidth,buttonHeight), "Host Game",button))
			{
				SwitchScreen(ScreenType.HostGame);
			}
			buttonI+=1;

            if (GUI.Button(new Rect(x, y + (buttonI * (buttonHeight + buttonDivider)), buttonWidth, buttonHeight), "Join Game", button))
			{
				SwitchScreen(ScreenType.FindGame);
			}
			buttonI+=1;

            if (GUI.Button(new Rect(x, y + (buttonI * (buttonHeight + buttonDivider)), buttonWidth, buttonHeight), "Create Profile", button))
			{
				this.SwitchScreen(ScreenType.CreateProfile);
			}
			buttonI+=1;

            if (GUI.Button(new Rect(x, y + (buttonI * (buttonHeight + buttonDivider)), buttonWidth, buttonHeight), "Select Profile", button))
            {
                this.SwitchScreen(ScreenType.SelectProfile);
            }
            buttonI += 1;

            if (GUI.Button(new Rect(x, y + (buttonI * (buttonHeight + buttonDivider)), buttonWidth, buttonHeight), "Edit Character", button))
            {
                this.SwitchScreen(ScreenType.EditCharacter);
            }
            buttonI += 1;

            if (GUI.Button(new Rect(x, y + (buttonI * (buttonHeight + buttonDivider)), buttonWidth, buttonHeight), "Options", button))
			{
				CurrentScreen = ScreenType.Options;
			}
			buttonI+=1;

            if (GUI.Button(new Rect(x, y + (buttonI * (buttonHeight + buttonDivider)), buttonWidth, buttonHeight), "Quit", button))
			{
				Application.Quit();
			}
	...
	public void SwitchScreen(ScreenType s)
	{
		...
		case ScreenType.HostGame:
			...
			break;
        case ScreenType.EditCharacter:
            menuEditCharacter = new MenuEditCharacter(this);
            break;
		}
